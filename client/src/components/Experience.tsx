...
const Experience = () => {
  ...
  return (
    <section id="experience" className="py-20 bg-slate-900/70" ref={sectionRef}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-16">
          {resumeData.experience.map((exp) => (
            <div 
              key={exp.id} 
              className="animate-on-scroll neon-card rounded-xl p-8 bg-slate-900/55"
              style={{ 
                opacity: 0, 
            ...
            >
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
                <div>
                  ...
                  <p className="text-xl text-cyan-300 font-semibold" data-testid={`company-${exp.id}`}>
                    {exp.company}
                  </p>
                ...
                {exp.current && (
                  <div className="mt-4 lg:mt-0">
                    <span className="inline-flex items-center px-4 py-2 bg-cyan-300/15 text-cyan-200 rounded-full text-sm font-semibold border border-cyan-300/45">
              ...
              <div className="space-y-8">
                {exp.projects.map((project, projectIndex) => (
                  <div key={project.id} className="bg-slate-900/70 border border-slate-700 rounded-lg p-6 neon-card" data-testid={`project-${project.id}`}>
                    <h4 className="text-lg font-bold text-cyan-200 mb-2 flex items-center">
